...
const dateSchema = z.object({
  dateValue: z.string().refine((data) => !isNaN(Date.parse(data)), { message: "Invalid date format" }),
})
...
export const DateField: FieldType<DateData> = {
  renderAdmin: ({ data, setData }: FieldTypeProps<DateData>): ReactElement => {
    return <Input type="date" defaultValue={data?.dateValue || ""} onChange={(e) => setData && setData({ dateValue: e.target.value })} />
  },
